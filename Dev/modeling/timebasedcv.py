...
class TimeBasedCV(object):
    ...
    def split(self, data, validation_split_date=None, date_column="record_date", gap=0):
        ...
        if validation_split_date == None:
            validation_split_date = data[date_column].min() + eval(
        ...
        while end_test < data[date_column].max():
            ...
            cur_train_indices = list(
                data[
                    (data[date_column] >= start_train) & (data[date_column] < end_train)
                ].index
            )
            ...
            cur_test_indices = list(
                data[
                    (data[date_column] >= start_test) & (data[date_column] < end_test)
                ].index
            )
